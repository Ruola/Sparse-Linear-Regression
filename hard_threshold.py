...
class HardThreshold:
    ...
    def hard_threshold_algo(self, y,H,lambda_para,alpha,N, SIGMA, x_true):
    ...
    def run_hard_threshold(self, x_original, y, H, N, SIGMA):
        #alpha = 5. * math.ceil(max(np.linalg.eigh(np.dot(H.T, H))[0]))
        alpha = 1000
        # print("alpha = ", alpha)
        lambda_para = 1
        x, objective_funcs, pred_errors,classi_errors = self.hard_threshold_algo(y,H, lambda_para,alpha,N, SIGMA, x_original)
