...
class Ista:
    ...
    def soft_threshold(self, y, H, lambda_para, alpha, N):
        ...
        for _ in range(N):
            ...
            # soft(temp, lambda)
            temp = x + np.dot(np.transpose(H),
                              y - np.dot(H, x)) / alpha / len(y)
            x = temp - lambda_para * np.sign(temp)
            x[x * np.sign(temp) < 0] = 0
        return x

    def ista(self, y, H, lambda_para, alpha, N, SIGMA_half, x_true):
        # input:
        # y - observed signal
        # H - design matrix
        ...
        for i in range(N):
            ...
            gener_errors[i] = self.get_gener_error(x_true, x, SIGMA_half)
        return (x, objective_funcs, pred_errors, gener_errors)

    def run_ista(self, x_original, y, H, N, SIGMA_half):
        alpha = 2. * math.ceil(max(np.linalg.eigh(np.dot(H.T, H))[0])) / len(y)
        # print("alpha = ", alpha)
        # tuning lambda
        objective_f = self.get_objective
        algo_f = lambda _y, _H, _para: self.soft_threshold(
            _y, _H, _para, alpha, N)
        ...
        x, best_objective_funcs, pred_errors, gener_errors = self.ista(
            y, H, best_lambda, alpha, N, SIGMA_half, x_original)
        ...
        return x, best_lambda, best_objective_funcs, pred_errors, gener_errors
