...
class TestGradientDescent(unittest.TestCase):
    ...
    def test_insert_zero(self):
        ...
        x_removed = np.delete(np.copy(x), indices_removed)
        x_updated = GradientDescent()._insert_zero(x_removed, indices_removed)
        x_expected = np.asarray([2, 3, 0, 2, 0, 4, 5, 0])
    ...
    def test_update_signal_estimation(self):
        ...
        x_updated = GradientDescent().update_signal_estimation(
            x, y, H, lambda_step, gd_type, 1, iter_type)
